...
@app.get("/api/search_file/{alias}", response_model=FileResponse)
...
@app.post("/api/search_files/batch", response_model=BatchResponse)
...
@app.get("/api/search_tag", response_model=TagSearchResponse)
...
async def not_found_handler(request, exc):
    return ErrorResponse(error="Not Found", detail=str(exc.detail))

@app.exception_handler(500)
async def internal_error_handler(request, exc):
    return ErrorResponse(error="Internal Server Error", detail=str(exc.detail))
